...
import JoinConferenceModal from "../components/dashboard/JoinConferenceModal";
...
const DashboardPage = () => {
    ...
    const [useMediasoup, setUseMediasoup] = useState(
        localStorage.getItem("useMediasoup") === "true",
    );
    ...
    const handleJoinConference = async (readableId) => {
        try {
            ...
            const route = useMediasoup
                ? `/conference-ms/${result.conference.id}`
                : `/conference/${result.conference.id}`;
            navigate(route);
        } catch (error) {
    ...
    const toggleMediaServer = () => {
        const newValue = !useMediasoup;
        setUseMediasoup(newValue);
        localStorage.setItem("useMediasoup", newValue.toString());
    };
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex-1">
                            ...
                            <div className="mt-3 flex items-center gap-2">
                                <button
                                    onClick={toggleMediaServer}
                                    className="flex items-center gap-2 px-3 py-1.5 bg-indigo-700 bg-opacity-50 rounded-lg text-sm hover:bg-opacity-70 transition-colors"
                                >
                                    <Settings className="w-4 h-4" />
                                    Media:{" "}
                                    {useMediasoup ? "MediaSoup" : "Custom SFU"}
                                </button>
                            </div>
                        </div>
